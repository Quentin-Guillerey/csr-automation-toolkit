...
optional Slack alerting and optional AI fallback classification. Every
suggested response is reviewable and editable by the agent before it's
sent — nothing is logged until the agent confirms with "Send". The CSV
audit trail records both the system's suggestion and the agent's final
text side by side, tagged with the matched category, so edits can be
reviewed later as a signal for improving the keyword rules.
...
def get_response(query):
    ...
    category_key is the response_key from STANDARD_RESPONSES (e.g. "cwsh"),
    or "uncategorized" when nothing matched. Carried through to the log so
    that edited responses can be traced back to the category that produced
    the original suggestion — needed for future rule-building, not used
    for matching itself.
    """
    query_lower = query.lower()

    for keywords, response_key, response_text, follow_up in STANDARD_RESPONSES:
        if any(keyword in query_lower for keyword in keywords):
            return response_text, follow_up, response_key
    ...
    return "Thank you for reaching out. I'll do my best to assist you.", False, "uncategorized"


def log_to_csv(query, suggested_response, final_response, category="uncategorized",
                customer_email=None, follow_up=False):
    """
    ...
    Logs both the system's suggested response and what was actually sent.
    When they differ, the row is a human correction — tagged with the
    category the system originally matched, so a later review pass can
    find "category X keeps getting overridden" patterns without having to
    re-run the classifier against old queries.
    """
    file_path = get_log_path()
    follow_up_text = "Yes" if follow_up else "No"
    was_edited = "Yes" if final_response.strip() != suggested_response.strip() else "No"
    new_row = [
    ...
    try:
        with open(file_path, mode="a", newline="", encoding="utf-8") as f:
            ...
            if not file_exists:
                writer.writerow([
                    "Timestamp", "Customer Email", "Query", "Suggested Response",
                    "Final Response", "Was Edited", "Category", "Follow-Up Needed",
...
class CSRAutomationApp:
    """Desktop GUI for CSR agents to input queries and review/send auto-responses.

    Flow: Generate Response fills the box with the system's suggestion.
    The agent can edit it freely before sending — nothing is logged until
    "Send" is clicked. This is deliberate: Generate is a draft step, Send
    is the confirmation that produces the audit record. Comparing the
    suggested text to the box's contents at Send time is how "Was Edited"
    gets set — no separate flag for the agent to remember to toggle.
    """
    ...
    def __init__(self, root):
        ...
        self.current_suggested_response = ""
        ...
        tk.Label(root, text="Response (editable — review before sending):",
        ...
        tk.Button(button_frame, text="Generate Response",
                  command=self.generate_response, bg="#2196F3", fg="white").pack(side=tk.LEFT, padx=5)
        self.send_button = tk.Button(button_frame, text="Send",
                  command=self.send_response, bg="#4CAF50", fg="white", state=tk.DISABLED)
    ...
    def generate_response(self):
        ...
        self.current_suggested_response = response
        ...
        self.status_label.config(text="Suggestion ready — review, edit if needed, then Send.", fg="blue")
    ...
    def send_response(self):
        ...
        query = self.query_input.get("1.0", tk.END).strip()
        final_response = self.response_output.get("1.0", tk.END).strip()
        if not final_response:
            messagebox.showerror("Error", "Response can't be empty.")
            return
        ...
        log_to_sheets(query, final_response, follow_up=self.current_follow_up)
        send_slack_alert(query, final_response, self.current_follow_up)

        was_edited = final_response != self.current_suggested_response.strip()
        if was_edited:
            self.status_label.config(text="Sent with edits — logged for review.", fg="#b8860b")
        else:
            self.status_label.config(text="Sent as suggested — logged.", fg="blue")
