...
service-account auth. Response library externalized to responses.json
(user-editable, seeded to the config folder on first run) with an in-app
Reload button — the hardcoded STANDARD_RESPONSES list is gone.
"""
...
# ---------------------------------------------------------------------------
# Response library (responses.json)
#
# Responses live in an external JSON file, not in code, so non-technical
# teams can add or edit entries without touching Python or rebuilding the
# .exe. Load order:
#   1. User-editable copy in the per-user config folder (seeded from the
#      bundled default on first run) — survives app updates.
#   2. Bundled default shipped inside the .exe (or next to this script
#      when running from source).
#   3. If neither loads, the app still runs: every query gets the generic
#      fallback response and the failure is written to csr_errors.log.
# Entry order in the file matters: first match wins.
# ---------------------------------------------------------------------------

FALLBACK_RESPONSE = ("Thank you for reaching out. I'll do my best to assist you.", False)


def get_bundled_responses_path():
    """responses.json shipped with the app: inside the PyInstaller bundle
    when frozen, next to this script when running from source."""
...
def get_user_responses_path():
    """The editable copy each user actually loads from (config folder)."""
    return get_config_path().parent / "responses.json"


def _validate_entries(raw_entries):
    """Keep well-formed entries, skip and log the rest — one bad entry
    shouldn't take down the whole library."""
    entries = []
    for i, e in enumerate(raw_entries):
        ...
        if (not isinstance(keywords, list) or not keywords
            ...
            log_error(f"responses.json: skipping malformed entry #{i} (id={e.get('id', '?')})")
            continue
        entries.append((
            [k.lower() for k in keywords],
            e.get("id", ""),
            response,
            bool(e.get("follow_up", False)),
        ))
    return entries


def load_responses():
    """Returns (entries, source_path). Seeds the user copy from the
    bundled default on first run so there's always a visible file to edit."""
    user_path = get_user_responses_path()
    bundled_path = get_bundled_responses_path()

    if not user_path.exists() and bundled_path.exists():
        try:
            shutil.copyfile(bundled_path, user_path)
        except OSError as e:
            log_error(f"Could not seed editable responses.json: {e}")
    ...
    for path in (user_path, bundled_path):
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            entries = _validate_entries(data.get("responses", []))
            if entries:
                return entries, path
            log_error(f"{path}: no valid entries found.")
        except (json.JSONDecodeError, OSError) as e:
            log_error(f"Failed to load {path}: {e}")

    log_error("No response library loaded — only the generic fallback will be used.")
    return [], None


RESPONSES, RESPONSES_SOURCE = load_responses()


def reload_response_library():
    """Re-read responses.json (e.g. after a teammate edits it) without
    restarting the app. Returns the number of entries loaded."""
    global RESPONSES, RESPONSES_SOURCE
    RESPONSES, RESPONSES_SOURCE = load_responses()
...
def get_response(query):
    ...
    for keywords, _, response_text, follow_up in RESPONSES:
...
class CSRAutomationApp:
    ...
    def __init__(self, root):
        ...
        tk.Button(button_frame, text="Reload Responses",
                  command=self.reload_responses, bg="#2196F3", fg="white").pack(side=tk.LEFT, padx=5)
        ...
        mode_bits = []
        mode_bits.append(f"{len(RESPONSES)} responses")
    ...
    def reload_responses(self):
        count = reload_response_library()
        source = RESPONSES_SOURCE.name if RESPONSES_SOURCE else "no file found"
        color = "blue" if count else "#b00000"
        self.status_label.config(text=f"Reloaded {count} responses ({source}).", fg=color)
