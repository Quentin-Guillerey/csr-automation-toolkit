...
3. `requirements.txt` needs neither `python-dotenv` (replaced by the config
   screen) nor `oauth2client` (replaced by gspread's native service-account
   auth). If either is still listed locally, remove it.
...
## Diagnostics: where errors go

The .exe is built with `console=False`, so nothing printed to stdout is ever
visible. Every failure is written to:

```
%APPDATA%\CSRToolkit\csr_errors.log
```

and surfaced in the app's status bar. If a teammate reports "it said sent but
nothing arrived in Slack," that file is the first place to look. It records
webhook failures, Sheets auth errors, AI fallback problems, and any aborted
CSV write.

**One failure mode worth knowing:** if the audit log's schema has changed and
the old `csr_logs.csv` is open in Excel, the toolkit cannot rotate it. Rather
than append mismatched columns to the old file, it aborts the write, shows a
red status message, and pops an error dialog. Close the CSV and retry.
